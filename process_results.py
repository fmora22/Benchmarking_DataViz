...
def calculate_metrics(runs_data, summary_data, meta_data, device_name=None):
    """Calculate all metrics needed for the table."""
    
    model_key = meta_data.get('model_key', 'unknown')
    host = meta_data.get('host', 'unknown')
    
    # Use device_name from directory structure if provided
    # Otherwise fall back to hostname mapping
    if device_name:
        device = device_name
    else:
        # Map hostnames to device names (fallback)
        device_mapping = {
            'thorwaggle': 'Jetson Thor',
            'thorwaggle1': 'Jetson Thor',
            'dellgb10': 'Dell Pro Max',
            'dgxwaggle': 'Dell Pro Max',
            'ubuntu': 'Jetson AGX Orin',
            'waggle': 'Jetson AGX Orin',
...
def find_benchmark_runs(base_dir):
    ...
    device_mapping = {
        'dell_output': 'Dell Pro Max',
        'thor_output': 'Jetson Thor',
        'orin_output': 'Jetson AGX Orin',
    }
    
    # Look for device_output directories
    for device_dir in base_path.iterdir():
        if not device_dir.is_dir():
            continue
        
        # Get device name from folder
        device_name = device_mapping.get(device_dir.name, device_dir.name)
        
        # Now look for model directories within device folder
        for model_dir in device_dir.iterdir():
            if not model_dir.is_dir():
                continue
            
            # Look for run directories within model folder
...
def generate_latex_table(all_metrics):
    ...
    devices = sorted(set(m['device'] for m in all_metrics))  # Use actual devices found
    dtypes = ['bf16', 'fp16']
    ...
    lines.append(r"\textbf{Metrics:} Latency = mean per-image inference time. Throughput = images per second. "
                 r"GPU\% = average GPU compute utilization (via pynvml on Dell Pro Max; not available on Jetson devices). "
                 r"CPU\% = average CPU utilization. RAM = average system memory used. "
                 r"Power = average power draw (Jetson devices use tegrastats GPU+SOC, Dell uses pynvml GPU power). "
...
def main():
    ...
    for run_info in runs:
        print(f"Processing {run_info['model']} on {run_info['device']} - {run_info['run_group']}...")
        ...
        try:
            ...
            metrics = calculate_metrics(runs_data, summary_data, meta_data, device_name=run_info['device'])
